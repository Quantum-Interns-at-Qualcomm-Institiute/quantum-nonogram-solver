...
def render_chart_b64(
    ...
    fig.suptitle("Solver Benchmark Comparison", color=FG_MAIN, fontsize=12,
                 fontweight="bold")
    ...
    if cl:
        ...
        errs.append(0 if not cl_times or len(cl_times) < 2
                    else statistics.stdev(cl_times))
    if qu:
        labels.append("Quantum")
        vals.append(qu.solve_time_s)
        colors.append(BTN_QU)
        errs.append(0 if not qu_times or len(qu_times) < 2
                    else statistics.stdev(qu_times))
    if labels:
        bars = ax1.bar(labels, vals, color=colors, width=0.4,
                       yerr=errs if any(e > 0 for e in errs) else None,
                       capsize=5, error_kw={"color": FG_MAIN})
        ax1.set_ylabel("seconds", color=FG_MAIN, fontsize=8)
        for bar, v in zip(bars, vals):
            ax1.text(bar.get_x() + bar.get_width() / 2, v * 1.03,
                     f"{v:.4f}s", ha="center", va="bottom",
                     color=FG_MAIN, fontsize=7)
    ...
    if qu:
        ...
        q_vals = [
            qu.num_qubits, qu.circuit_depth, qu.total_gate_count,
            qu.two_qubit_gate_count, qu.grover_iterations,
        ]
        ...
        for bar, v in zip(bars, q_vals):
            ax2.text(bar.get_x() + bar.get_width() / 2, v + 0.05,
                     str(v), ha="center", va="bottom", color=FG_MAIN, fontsize=7)
    else:
        ax2.text(0.5, 0.5, "Quantum solver not run",
                 ha="center", va="center", color=FG_DIM, fontsize=9,
