...
from tools.state import state, state_lock, emit_status
...
def api_hw_backends():
    ...
    try:
        ...
        return jsonify({"backends": [
            {"name": b[0], "qubits": b[1], "pending": b[2]}
            for b in backends
        ]})
...
def api_hw_config():
    ...
    if data is None or data.get("disconnect"):
        ...
        with state_lock:
            state["hw_config"] = cfg
        _state_mod.socketio.emit("hw_status", {
            "connected": True,
        ...
        emit_status(
            ...
            f"\u2014 real quantum jobs may take several minutes.", "warn")
